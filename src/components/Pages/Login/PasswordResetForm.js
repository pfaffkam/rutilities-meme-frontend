...
function PasswordResetForm({ texts, setShowPasswordReset }) {
  return (
    <form className="bg-gray-700 p-8 flex flex-col justify-center items-center rounded-lg">
      <h2 className="text-lg text-white font-bold mb-4">{texts.remindPassword}</h2>
      ...
      <button type="button" className="text-white mt-2 cursor-pointer" onClick={() => setShowPasswordReset(false)}>
        {texts.back}
      </button>
    </form>
  );
